...
export function buildSamguk() {
    ...
    // ================================================================
    // 이 자리에 마을은 없다.
    // 성은 쌓였지만 사람은 아직 들어오지 않았다 — 집을 세우는 것은 나다.
    // ================================================================
    ...
    // 사람과 짐승
    // ================================================================
    // 성을 쌓는 중이다. 여기 있는 사람은 전부 그 공사와 얽혀 있다.
    //
    // 넷이 아는 것을 합치면 한 문장이 된다 —
    // 누군가 성벽을 일부러 꺾어, 옛 돌을 덮지 않았다.
    // 마지막 한 사람(아이)이 그 돌이 언제부터 있었는지를 흘린다.
    // 그 말이 플레이어를 청동기로 데려간다.

    // ---- 문지기: 이름을 안다. 제일 먼저 만나게 되는 사람 ----
    q = P(4, -12.2);
    addWorker(q[0], q[1], "samguk", {
        name: "성문 지기",
        rot: 0.15,
        greeting: "성은 아직 다 안 쌓였소. 무슨 일로 왔소?",
        choices: [
            {
                text: "기와 뒷면의 이름을 보았습니다. 누구입니까?",
                response: "달구(達仇). 이 성의 돌을 맡은 사람이오. "
            ...
            {
                text: "성벽이 저기서 왜 꺾여 있습니까?",
                response: "나야 모르오. 도면대로 쌓았겠지. "
                    + "...동쪽 끝은 달구가 직접 봤소. 그 늙은이한테 물어보시오.",
                journal: true,
                followUp: "(문지기가 어깨를 으쓱한다)"
            },
            {
                text: "성이 언제 완성됩니까?",
                response: "돌이 모자라오. 사람도 모자라고. "
                    + "장부에는 넉넉히 적혀 있다는데, 그 장부를 내가 본 적이 없소.",
                followUp: "(문지기가 한숨을 쉰다)"
            }
        ]
    });

    // ---- 대장장이: 목간의 마지막 줄을 쓴 사람이 누군지 안다 ----
    q = P(-10.4, -6.2);
    addWorker(q[0], q[1], "samguk", {
        name: "대장장이",
        rot: -0.8,
        greeting: "불이 죽었소. 돌 나르는 데 사람을 다 빼갔으니.",
        choices: [
            {
                text: "목간의 마지막 줄만 글씨가 다릅니다.",
                response: "그건 내가 쓴 게 아니오. 장부는 내가 적지만, "
                    + "그 줄은 어느 날 아침에 이미 거기 있었소. "
            ...
            {
                text: "달구라는 사람을 압니까?",
                response: "알다마다. 돌은 잘 다루는데 말이 없소. "
                    + "요새는 동쪽에만 붙어 있습디다. 제 무덤이라도 파는 사람처럼.",
                journal: true,
                followUp: "(대장장이가 헛웃음을 친다)"
            }
        ]
    });

    // ---- 달구: 성벽이 꺾인 그 자리에 있다. 목간을 본 뒤에야 입을 연다 ----
    q = P(22.5, -7.4);
    addWorker(q[0], q[1], "samguk", {
        name: "돌 맡은 이",
        rot: 2.4,
        greeting: "여기는 볼 게 없소. 가시오.",
        choices: [
            {
                text: "당신이 달구입니까?",
                response: "...누가 그럽디까.",
                journal: true,
                followUp: "(노인이 돌에서 손을 떼지 않는다)"
            },
            {
                text: "\"동쪽 끝은 돌아서 쌓으라.\" 이 말을 아십니까?",
                response: "그 줄을 봤소? ...그렇소. 내가 썼소. "
                    + "밤에 장부에 손을 댔소. 벌을 받겠지. "
            ...
            {
                text: "우리가 놓은 게 아니라면, 누가 놓았습니까?",
                response: "내 할아비의 할아비도 저 돌을 보고 자랐다 하오. "
    ...
    // ---- 아이: 어른들이 하지 않는 말을 한다. 청동기로 가는 갈고리 ----
    q = P(16.5, 3.4);
    addVillager([q, P(19, 5), P(14, 6), P(17, 2)], "samguk", {
        name: "성 밑 아이",
        speed: 1.5,
        greeting: "아저씨도 저 돌 보러 왔어요? 다들 저 돌만 봐요.",
        choices: [
            {
                text: "저 돌이 뭔지 아니?",
                response: "몰라요. 근데 할아버지가 그랬는데, "
                    + "아주 옛날에 저기서 사람들이 밤에 모였대요. 불을 켜고요. "
                    + "그때는 성도 없고 아무것도 없었대요.",
                clue: "samguk_child",
                journal: true,
                hideAfter: true,
            ...
            {
                text: "여기서 뭐 하고 있었니?",
                response: "고분에서 미끄럼 타요. 어른들은 하지 말래요. "
                    + "근데 어른들도 어렸을 때 다 했대요.",
                followUp: "(아이가 씩 웃는다)"
            }
        ]
    ...
    // ---- 개 한 마리. 성문 근처를 돈다 ----
    addDog([P(2, -10), P(7, -11), P(5, -7), P(0, -8)]);
